...
class TestDeepEPStreamingArgs(CustomTestCase):
    ...
    def test_streaming_accepts_topk_smaller_than_ep_size(self):
        hf_config = SimpleNamespace(
            quantization_config={
                "quant_method": "fp8",
                "activation_scheme": "dynamic",
        ...
        with envs.SGLANG_ENABLE_DEEPEP_STREAMING.override(False):
            server_args = self._args(ep_size=8)
            server_args.model_path = "/model/dsv4-fp8"
            with patch.object(
                server_args, "get_model_config", return_value=model_config
            ):
                server_args._handle_a2a_moe()
    ...
    def test_streaming_accepts_ep4_topology(self):
        with envs.SGLANG_ENABLE_DEEPEP_STREAMING.override(False):
            server_args = self._args(tp_size=4, dp_size=4, ep_size=4)
            server_args._handle_a2a_moe()
            self.assertTrue(server_args.enable_deepep_streaming)

    def test_streaming_rejects_unsupported_ep2_topology(self):
        with envs.SGLANG_ENABLE_DEEPEP_STREAMING.override(False):
            server_args = self._args(tp_size=2, dp_size=2, ep_size=2)
            with self.assertRaisesRegex(ValueError, "sizes of 4 or 8"):
                server_args._handle_a2a_moe()
