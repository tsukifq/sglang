...
/// PUB/SUB sockets need a short propagation window after a subscription is
/// installed.  Without it, the first engine event can win the "slow joiner"
/// race and the router's exact cache view is incomplete from request one.
const SUBSCRIPTION_SETTLE_TIME: Duration = Duration::from_millis(100);
...
struct SubscriberHandle {
    ...
    join: JoinHandle<()>,
    /// True only while connect + subscribe has completed and the receive
    /// loop is alive.  This is deliberately distinct from membership in the
    /// handles map: a configured task is not necessarily a usable stream.
...
impl KvEventSubscriberRegistry {
    ...
    /// Whether a concrete rank's SUB task completed its startup handshake
    /// and is still running.
    pub async fn is_ready(&self, worker: &KvWorkerId) -> bool {
        let handles = self.inner.handles.lock().await;
        handles.get(worker).is_some_and(|handle| {
            handle.ready.load(Ordering::Acquire) && !handle.join.is_finished()
        })
    }

    /// Wait for a rank stream to become usable. This is used only during
    /// lazy policy bootstrap, never on the steady-state routing hot path.
    pub async fn wait_ready(&self, worker: &KvWorkerId, timeout: Duration) -> bool {
        let deadline = tokio::time::Instant::now() + timeout;
        loop {
            if self.is_ready(worker).await {
                return true;
            }
...
fn spawn_subscriber_task(
    ...
    tokio::spawn(async move {
        run_subscriber(id, endpoint, topic, tx, cancel, ready).await;
    })
}
...
async fn run_subscriber(
    ...
    // Give the subscription enough time to propagate before exposing this
    // stream to exact cache policies. Cancellation still wins immediately.
    tokio::select! {
        _ = cancel.cancelled() => return,
        _ = tokio::time::sleep(SUBSCRIPTION_SETTLE_TIME) => {}
    }
    ready.store(true, Ordering::Release);
    let _ready_guard = ReadyGuard(ready);
...
mod tests {
    ...
    #[tokio::test]
    async fn readiness_requires_live_subscriber_task() {
        let (_pub_sock, port) = helpers::make_pub_bound().await;
        let (tx, _rx) = mpsc::channel::<WorkerEvent>(8);
        let registry = KvEventSubscriberRegistry::new(tx);
        let worker = KvWorkerId::new("http://127.0.0.1:30000".to_string(), 0);

        registry
            .add_worker(&worker.url, &helpers::cfg_for(&worker.url, port, 1))
            .await;
        assert!(
            registry
                .wait_ready(&worker, Duration::from_millis(500))
                .await,
            "connect + subscribe should make the rank ready"
        );

        registry.shutdown().await;
        assert!(!registry.is_ready(&worker).await);
    }
