...
impl TreeState {
    ...
    /// Return the longest leading chain held by one concrete DP-rank worker.
    ///
    /// `match_prefix` returns only the holder set at the globally deepest
    /// node.  Rank-level cost functions such as LMetric need a score for
    /// every candidate, including ranks that hold a shorter prefix, so they
    /// cannot recover their inputs from that aggregate result.
    fn match_prefix_for_worker(&self, worker: &KvWorkerId, block_hashes: &[i64]) -> usize {
        ...
        for &hash in block_hashes {
            ...
            if !child.workers.contains(worker) {
                break;
            }
            child.last_used.store(now, Ordering::Relaxed);
            current = child_id;
...
impl HashTree {
    ...
    /// Return the number of leading blocks resident on one DP-rank worker.
    pub fn match_prefix_for_worker(&self, worker: &KvWorkerId, block_hashes: &[i64]) -> usize {
        let state = self.state.read();
        state.match_prefix_for_worker(worker, block_hashes)
    }
...
mod tests {
    ...
    #[test]
    fn per_worker_match_preserves_shorter_rank_score() {
        let tree = HashTree::new();
        let rank0 = worker("http://engine", 0);
        let rank1 = worker("http://engine", 1);
        tree.insert(&rank0, None, &[10, 20, 30, 40]);
        tree.insert(&rank1, None, &[10, 20]);

        // Aggregate lookup exposes only the globally deepest holders.
        let aggregate = tree.match_prefix(None, &[10, 20, 30, 40]);
        assert_eq!(aggregate.matched_blocks, 4);
        assert_eq!(aggregate.workers, workers(&[&rank0]));

        // Rank cost functions still recover rank1's useful shorter prefix.
        assert_eq!(tree.match_prefix_for_worker(&rank0, &[10, 20, 30, 40]), 4);
        assert_eq!(tree.match_prefix_for_worker(&rank1, &[10, 20, 30, 40]), 2);
    }
}
