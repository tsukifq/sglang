...
/// Upper bound for lazy policy startup to establish one rank's KV stream.
const SUBSCRIBER_READY_TIMEOUT: Duration = Duration::from_secs(2);
...
impl KvEventIndex {
    ...
    /// Whether discovery and subscriber setup succeeded for this base URL.
    pub fn has_worker(&self, worker_url: &str) -> bool {
        self.workers.lock().contains_key(worker_url)
    }

    /// Whether one concrete DP-rank stream is part of the live index.
    pub fn is_live(&self, worker: &KvWorkerId) -> bool {
        self.live_workers.lock().contains(worker)
    }

    /// Wait until one concrete rank has both been discovered and completed
    /// its ZMQ connect + subscribe startup. Membership in `live_workers`
    /// alone is insufficient because ranks are marked live before tasks are
    /// spawned to avoid dropping their first event in the pump.
    pub async fn wait_ready(&self, worker: &KvWorkerId) -> bool {
        self.is_live(worker)
            && self
                .subscribers
                .wait_ready(worker, SUBSCRIBER_READY_TIMEOUT)
                .await
    }
